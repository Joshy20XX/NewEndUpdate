...
public class NewEndUpdateModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, NewEndUpdateMod.MODID);
	public static final RegistryObject<Biome> ENDBIOTRY = REGISTRY.register("endbiotry", () -> EndbiotryBiome.createBiome());
		...
		for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
			...
			if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					for (Climate.ParameterPoint parameterPoint : EndbiotryBiome.PARAMETER_POINTS) {
						parameters.add(new Pair<>(parameterPoint, biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, ENDBIOTRY.getId()))));
					}
				...
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					...
					if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
						...
						surfaceRules.add(1, preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, ENDBIOTRY.getId()), Blocks.AIR.defaultBlockState(), Blocks.END_STONE.defaultBlockState(), Blocks.END_STONE.defaultBlockState()));
